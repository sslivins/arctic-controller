...
class TestEventsAPI:
    ...
    def test_events_empty_after_clear(self):
        ...
        _delete("/api/events")
        # Read immediately: DELETE clears synchronously server-side, so the
        # freshest read shows the minimum count. Sleeping here only gave
        # background tasks time to log *more* events (flakier, not safer).
        data = _get("/api/events").json()
