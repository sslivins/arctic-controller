...
// ============================================================================
// POST /api/test/weather-mock — install a canned Open-Meteo forecast body so
// the status-bar weather resolves deterministically without hitting the
// network, then trigger a refresh so the status bar updates.
// Body: the raw Open-Meteo forecast response, e.g.
//   {"current":{"temperature_2m":-5.0,"weather_code":71}}
// A body containing "__error__" makes the fetch report a failure.
// ============================================================================

static esp_err_t weather_mock_post_handler(httpd_req_t* req)
{
    CHECK_SESSION_LOCK(req);
    ...
    weather_set_mock_json(buf);
    free(buf);

    // Kick a refresh so the mocked value lands in the status bar promptly.
...
static esp_err_t weather_mock_reset_post_handler(httpd_req_t* req)
{
    CHECK_SESSION_LOCK(req);
    weather_clear_mock();
...
void test_endpoints_register(httpd_handle_t server)
{
    ...
    httpd_uri_t weather_mock_uri = {
        .uri = "/api/test/weather-mock",
        .method = HTTP_POST,
        .handler = weather_mock_post_handler,
        .user_ctx = NULL
    };
    reg_test_uri(server, &weather_mock_uri);

    httpd_uri_t weather_mock_options_uri = {
        .uri = "/api/test/weather-mock",
        .method = HTTP_OPTIONS,
        .handler = test_options_handler,
        .user_ctx = NULL
    };
    reg_test_uri(server, &weather_mock_options_uri);

    httpd_uri_t weather_mock_reset_uri = {
        .uri = "/api/test/weather-mock-reset",
        .method = HTTP_POST,
        .handler = weather_mock_reset_post_handler,
        .user_ctx = NULL
    };
    reg_test_uri(server, &weather_mock_reset_uri);

    httpd_uri_t weather_mock_reset_options_uri = {
        .uri = "/api/test/weather-mock-reset",
        .method = HTTP_OPTIONS,
        .handler = test_options_handler,
        .user_ctx = NULL
    };
    reg_test_uri(server, &weather_mock_reset_options_uri);
