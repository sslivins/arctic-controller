...
| `host/port/include/os_wrapper.h`: `MEM_ALLOC` uses `heap_caps_malloc(..., MALLOC_CAP_DMA \| MALLOC_CAP_CACHE_ALIGNED \| MALLOC_CAP_SPIRAM)` with an internal-RAM fallback | IDF 6 **removed** `esp_dma_capable_malloc()` / `esp_dma_mem_info_t`. This is the replacement Espressif documents in `migration-guides/release-6.x/6.0/peripherals.rst`. Preserves patch 0001's PSRAM preference. |
